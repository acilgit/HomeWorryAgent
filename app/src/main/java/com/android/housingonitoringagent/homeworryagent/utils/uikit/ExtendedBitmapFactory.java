...
import com.android.housingonitoringagent.homeworryagent.cache.FileManager;
import com.android.housingonitoringagent.homeworryagent.utils.FileUtil;
import com.android.housingonitoringagent.homeworryagent.utils.UriUtil;
...
public class ExtendedBitmapFactory {
    ...
    public static Uri compressImage(Context context, Uri uri, int maxWidth, int maxHeight) throws IOException {
        if (uri == null) {
            return null;
        }
        Bitmap bitmap = ExtendedBitmapFactory.createBitmapFromUri(context, uri);

        // 缩小图片到合适尺寸
        Bitmap scaleBmp = ExtendedBitmapFactory.shrink(bitmap, maxWidth, maxHeight);

        // 对图片进行压缩，并转换为byte[]
        byte[] bytes = ExtendedBitmapFactory.toByteArray(scaleBmp);

        // 保存压缩后的图片
        File file = new File(FileManager.compressDir(), UUID.randomUUID() + ".png");
        if (!FileUtil.saveFile(file, bytes)) {
            return null;
        }
    ...
    public static String toBase64(Context context, Uri uri) throws IOException {
        if (uri == null) {
            return null;
        }

        String extension = UriUtil.getExtensionFromMimeType(context, uri);
        if (TextUtils.isEmpty(extension)) {// 如果扩展名为空，默认赋值为png
            extension = "png";
        }
        byte[] bytes = UriUtil.getBytes(context, uri);
        String imageStr = Base64.encodeToString(bytes, Base64.DEFAULT);

        return ExtendedBitmapFactory.formatToBase64(extension, imageStr);
    }
